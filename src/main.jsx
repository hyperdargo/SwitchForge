...
function AdminPanel({ onClose, initialTab='gateway' }) {
  const [tab,setTab]=useState(initialTab),[config,setConfig]=useState(null),[apiKey,setApiKey]=useState(''),[models,setModels]=useState([]),[users,setUsers]=useState([]),[busy,setBusy]=useState(false),[message,setMessage]=useState('')
  useEffect(()=>{api('/admin/gateway').then(data=>setConfig(data.gateway)).catch(err=>setMessage(err.message));api('/admin/users').then(data=>setUsers(data.users)).catch(err=>setMessage(err.message))},[])
...
function Dashboard({ user, onLogout, onDocs }) {
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard">
      <header><button className="mobile-menu"><Menu/></button><div className="search"><Search/><span>Search documentation...</span><kbd>⌘ K</kbd></div><div className="header-right">{user.role==='admin'&&<button className="admin-header-btn" onClick={()=>setAdminSection('users')}><BarChart3/>Users</button>}<button className="docs-btn" onClick={onDocs}><BookOpen/>Docs</button><button className="avatar">{user.name.slice(0,2).toUpperCase()}</button></div></header>
