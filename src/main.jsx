...
function CreateKeyModal({ onClose, onCreate }) {
  ...
  return <div className="modal-bg" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><div className="modal">
    ...
    <div className="field-grid"><label>Expires after<select value={months} onChange={e=>setMonths(Number(e.target.value))}><option value="1">1 month</option><option value="2">2 months</option><option value="3">3 months</option></select><ChevronDown/></label><label>Token allowance<select value={limit} onChange={e=>setLimit(Number(e.target.value))}><option value="100000">100K tokens</option><option value="500000">500K tokens</option><option value="1000000">1M tokens</option></select><ChevronDown/></label></div>
    <div className="modal-note"><Clock3/>Keys automatically expire after 1–3 months. Create a new key when this one expires.</div>
...
function AdminPanel({ onClose }) {
  ...
  return <div className="modal-bg admin-bg"><div className="admin-panel"><header><div><div className="eyebrow"><span/> ADMINISTRATION</div><h2>Gateway control</h2><p>Manage the live OmniRoute connection and routing combos.</p></div><button className="modal-close" onClick={onClose}><X/></button></header>{!config?<div className="admin-loading">Loading gateway configuration...</div>:<div className="admin-form"><div className="admin-status"><ServerCog/><div><b>Active configuration</b><span>{config.source==='admin'?'Managed from this console':'Using server environment defaults'} · API key {config.apiKeyConfigured?'configured':'missing'}</span></div></div><label>OmniRoute base URL<input value={config.baseUrl} onChange={e=>update('baseUrl',e.target.value)} placeholder="https://route.example.com/v1"/></label><div className="admin-combos"><label>Free combo model<input value={config.freeModel} onChange={e=>update('freeModel',e.target.value)} placeholder="Normal Chat"/><small>First-priority route for ordinary conversation.</small></label><label>Premium combo model<input value={config.premiumModel} onChange={e=>update('premiumModel',e.target.value)} placeholder="Premium"/><small>Used for clear coding and technical prompts.</small></label></div><div className="aux-head"><div><h3>Auxiliary tasks</h3><p>Use <code>auto</code> for the main route, or enter a specific OmniRoute combo.</p></div><button className="secondary-btn" onClick={()=>setConfig(current=>({...current,auxiliary:Object.fromEntries(auxiliaryTasks.map(([id])=>[id,'auto']))}))}><RotateCcw/>Reset all to auto</button></div><div className="aux-list">{auxiliaryTasks.map(([id,label,description])=><div className="aux-row" key={id}><span><b>{label}</b><small>{description}</small></span><input value={config.auxiliary?.[id]||'auto'} onChange={e=>setConfig(current=>({...current,auxiliary:{...current.auxiliary,[id]:e.target.value}}))} aria-label={`${label} route`}/></div>)}</div><label>Replace OmniRoute API key<input type="password" autoComplete="new-password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="Leave blank to keep the current key"/></label>{message&&<div className="admin-message">{message}</div>}<div className="admin-actions"><button className="secondary-btn" disabled={busy} onClick={test}><Zap/>Test connection</button><button className="primary-btn" disabled={busy||!config.baseUrl||!config.freeModel||!config.premiumModel} onClick={save}><Save/>{busy?'Working...':'Save configuration'}</button></div></div>}</div></div>
}
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard">
      ...
      <div className="dash-body">{view==='usage'?<UsagePage/>:<>
        <div className="welcome"><div><div className="eyebrow"><span/> DEVELOPER CONSOLE</div><h1>Good morning, {user.name.split(' ')[0]}.</h1><p>Here's what's happening with your SwitchForge APIs.</p></div><button className="primary-btn" onClick={()=>keys.length<3?setModal(true):setToast('You can create a maximum of 3 API keys')}><Plus/>Create API key</button></div>
        <section className="stats-grid">
          <div className="stat-card"><div className="stat-top"><span className="stat-icon purple"><KeyRound/></span><small>API KEYS</small></div><strong>{keys.length}<em>/ 3</em></strong><p>{3-keys.length} key slot{3-keys.length===1?'':'s'} remaining</p></div>
